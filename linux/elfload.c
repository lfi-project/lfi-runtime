    ...
    struct Buf buf, size_t pagesize, size_t p_align, int map_op)
{
    ...
    if (map_op == ELF_MAP && buf.fd != -1) {
        ...
        if (memsz > filesz) {
            ...
            if (bss_rest < end) {
                p = lfi_box_mapat(box, p2l(box, bss_rest), end - bss_rest, prot,
        ...
        p = lfi_box_mapat_register(box, p2l(box, start), end - start, prot,
            LFI_MAP_PRIVATE | LFI_MAP_ANONYMOUS, -1, 0);
    }
    if (p == (lfiptr) -1) {
        return false;
...
elf_load(struct LFILinuxProc *proc, const char *prog_path, int prog_fd,
    ...
    int interp_fd, uint8_t *interp_data, size_t interp_size, int map_op,
    struct ELFLoadInfo *info)
{
    ...
    if (!elf_load_one(proc, prog, base, pagesize, map_op, &p_first,
